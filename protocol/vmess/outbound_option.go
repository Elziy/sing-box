...
func NewVMessNativeOutboundOption(content string) (option.Outbound, error) {
	vmessURL := betterdecode.DecodeBase64Safe(content)
	config, err := parseVMessURL(vmessURL)
	if err != nil {
		return option.Outbound{}, err
	}
	return NewVMessOutboundOption(config)
}

func NewVMessOutboundOption(config map[string]any) (option.Outbound, error) {
	...
	if cipher, exists := config["cipher"].(string); exists {
		options.Security = cipher
	}
	if aid, exists := config["alterId"].(int); exists {
		options.AlterId = aid
	}
	if globalPadding, exists := config["global-padding"].(bool); exists {
		options.GlobalPadding = globalPadding
	}
	if authenticatedLength, exists := config["authenticated-length"].(bool); exists {
...
func parseVMessURL(vmessURL string) (map[string]any, error) {
	vmessURL = betterdecode.DecodeBase64Safe(strings.TrimPrefix(vmessURL, "vmess://"))

	if config, err := parseVMessJSONParams(vmessURL); err == nil {
	...
	var name string
	if idx := strings.Index(vmessURL, "#"); idx != -1 {
		name = betterdecode.DecodeURIComponent(vmessURL[idx+1:])
		vmessURL = vmessURL[:idx]
	}

	var queryPart string
	if idx := strings.Index(vmessURL, "?"); idx != -1 {
		queryPart = vmessURL[idx+1:]
		vmessURL = vmessURL[:idx]
	}

	// uuid@server:port
	atIndex := strings.Index(vmessURL, "@")
	if atIndex == -1 {
		return nil, E.New("missing '@' separator")
	}

	config["uuid"] = vmessURL[:atIndex]
	serverPart := vmessURL[atIndex+1:]
	...
	if queryPart != "" {
		if err := parseVMessQueryParams(queryPart, &config); err != nil {
			return nil, err
		}
	}
...
func parseVMessQueryParams(queryPart string, config *map[string]any) error {
	...
	for _, param := range params {
		...
		switch key {
		case "security", "encryption", "scy":
			(*config)["cipher"] = value
		case "alter_id", "alterId", "aid":
			if alterId, err := strconv.Atoi(value); err == nil {
				(*config)["alterId"] = alterId
			}
		case "global_padding", "globalPadding":
			if value == "1" || value == "true" || value == "" {
				(*config)["global-padding"] = true
			}
		case "authenticated_length", "authenticatedLength":
			if value == "1" || value == "true" || value == "" {
				(*config)["authenticated-length"] = true
			}
		case "packet_encoding", "packetencoding":
			(*config)["packet-encoding"] = value
		case "xudp":
			if value == "1" || value == "true" || value == "" {
				(*config)["packet-encoding"] = "xudp"
			}
...
func parseVMessJSONParams(jsonStr string) (map[string]any, error) {
	config := make(map[string]any)
	...
	for key, value := range config {
		...
		case "host":
			...
			transport["headers"].(map[string]any)["Host"] = []string{fmt.Sprint(value)}
			// default
			transport["host"] = badoption.Listable[string]{fmt.Sprint(value)}
		//	ws
		case "ed":
			if ed, err := strconv.Atoi(fmt.Sprint(value)); err == nil {
				transport["max-early-data"] = ed
			}
		//	http
		case "method":
		...
		case "sni":
			...
			config["sni"] = value
		case "alpn":
			alpnList := strings.Split(fmt.Sprint(value), ",")
			for i, a := range alpnList {
				alpnList[i] = strings.TrimSpace(a)
			}
			config["tls"] = true
